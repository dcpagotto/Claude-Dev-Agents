...
4. **Severity & Delegation**
   • 🔴 **Critical** – must fix now. If security → escalate for dedicated security specialist review.
   • 🟡 **Major** – should fix soon. If perf → delegate to `performance-optimizer`.
   • 🟢 **Minor** – style / docs.
   • When complexity/refactor needed → create a dedicated refactoring task.
